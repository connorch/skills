...
function matchWord(key: string, word: string): { score: number; hits: number[] } | null {
...
export function searchFiles(files: FileEntry[], tokens: Token[], words: string[]): Match[] {
  ...
  scored.sort(
    (a, b) => a.score - b.score || b.match.file.uploaded.localeCompare(a.match.file.uploaded),
  );
...
export function suggestValues(files: FileEntry[], name: TokenName, partial: string): Suggestion[] {
