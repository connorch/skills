...
export function useDeleteFile(onDeleted?: (key: string) => void) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (key: string) => deleteFile(key).then(({ deleted }) => ({ key, deleted })),
    onSuccess: ({ key, deleted }) => {
      queryClient.removeQueries({ queryKey: fileQuery(key).queryKey });
      void queryClient.invalidateQueries({ queryKey: ["listing"] });
      void queryClient.invalidateQueries({ queryKey: ["files"] });
      toast.success(`deleted ${key} (${deleted.length} object${deleted.length === 1 ? "" : "s"})`);
      onDeleted?.(key);
    },
    onError: (cause) => toast.error(cause instanceof Error ? cause.message : "delete failed"),
  });
}
