...
export async function host(request: Request): Promise<HostResult> {
  ...
  if (url.pathname === "/api" || url.pathname.startsWith("/api/")) return api(request, url);
  ...
  if (request.method === "GET" || request.method === "HEAD") return resolve(request, url);
...
async function resolve(request: Request, url: URL): Promise<HostResult> {
  ...
  const archiveIndex =
    key && !trailingSlash && !isArchiveKey(key) ? key.match(/^(.+)\/archive$/) : null;
  ...
  if (versionInPath) return redirect(`/${fileKey}?version=${encodeURIComponent(stamp!)}`);
...
function fileResponse(request: Request, object: R2Object, publicObject: boolean): Response {
  ...
  if (request.method === "HEAD" || !hasBody(object)) return new Response(null, { headers });
