...
export function isPublic(key: string, meta: Record<string, string> | undefined): boolean {
...
export async function listFiles(params: URLSearchParams): Promise<Listing> {
  ...
  do {
    ...
    for (const dir of page.delimitedPrefixes ?? []) {
      ...
      if (!prefix && (dir === "archive/" || RESERVED_TOP_LEVEL.includes(dir.slice(0, -1))))
        continue;
...
export function listDirectory(prefix: string): Promise<Listing> {
  return listFiles(new URLSearchParams({ prefix, delimiter: "/", limit: "1000" }));
}
...
export async function versionsOf(key: string): Promise<Versions> {
  const [versions, current] = await Promise.all([listVersions(key), bucket().head(key)]);
  return {
