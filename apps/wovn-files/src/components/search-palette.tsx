...
// value autocomplete. Results are real links: Enter opens the File in this
// tab, ⌘-click in a new one, ⌘Enter copies its URL, → goes to its Directory
// Route.
        ...
        <CommandList className="max-h-96">
          ...
          {shown.length > 0 && (
            <CommandGroup heading={empty ? "recent" : "files"}>
              {shown.map((match) => (
                <FileItem key={match.file.key} match={match} />
              ))}
            </CommandGroup>
          )}
        </CommandList>
