...
export function Preview({ page }: { page: FilePage }) {
  ...
  switch (kindOf(shown.contentType, shown.size)) {
      ...
      return (
        <div className="flex justify-center p-4">
          <a href={src} className="max-w-full">
            <img src={src} alt={name} className="max-h-[calc(100vh-6rem)] max-w-full" />
          </a>
        </div>
      );
    case "video":
      return (
        <div className="flex justify-center p-4">
          <video src={src} controls className="max-h-[calc(100vh-6rem)] max-w-full" />
        </div>
      );
    case "audio":
      return (
    ...
    case "frame":
      return <iframe src={src} title={name} className="h-[calc(100vh-2rem)] w-full border-0" />;
    case "text":
...
function TextPreview({ src }: { src: string }) {
  ...
  if (text.data === undefined) return <div className="p-4 text-muted-foreground">loading</div>;
  return (
