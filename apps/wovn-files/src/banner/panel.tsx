  ...
  return (
    <div className="border-b border-border bg-background font-mono text-[12.5px]">
      ...
      <div className="max-h-[60vh] overflow-y-auto px-2.5 py-1.5">
        {tab === "files" && <DirectoryView prefix={prefix} currentKey={page.file.key} />}
...
function VersionsPane({ page }: { page: FilePage }) {
  ...
  const row = (href: string, label: string, size: number, uploaded: string, current: boolean) => (
        ...
        {page.versions.length === 0 && (
          <tr>
            <td className="py-2 text-muted-foreground" colSpan={3}>
              {page.file.stable ? "no previous versions" : "generated key, never overwritten"}
            </td>
          </tr>
        )}
      </tbody>
...
function DetailsPane({ page, file }: { page: FilePage; file: FileMeta }) {
  ...
  const rows: [string, React.ReactNode][] = [
    ...
    ["visibility", <VisibilityBadge file={file} readOnly={page.version !== null} />],
    ...
    ...(file.project ? [["project", file.project] as [string, React.ReactNode]] : []),
    ...(file.branch ? [["branch", file.branch] as [string, React.ReactNode]] : []),
    ...(file.worktree ? [["worktree", file.worktree] as [string, React.ReactNode]] : []),
        ...
        <tbody>
          {rows.map(([label, value]) => (
            <tr key={label} className="border-b border-border/50">
              <td className="py-1 pr-6 whitespace-nowrap text-muted-foreground">{label}</td>
          ...
          <tr>
            ...
            <td className="py-1 whitespace-nowrap">
              <button
                ...
                onClick={() =>
                  copyUrl(page.version ? versionUrl(file.key, stamp!) : fileUrl(file.key))
                }
              >
                copy url
              </button>
